...
import { getApiBaseUrl, isDesktopApp } from './utils/backendPort'
...
async function initializeApp() {
  ...
  try {
    // CRITICAL: Initialize backend connection BEFORE creating the app
    if (isDesktopApp()) {
      console.log('🖥️ Running in Tauri desktop mode')
      const baseUrl = await getApiBaseUrl()
      console.log('✅ Backend URL initialized:', baseUrl)
    } else {
