use crate::{error::ServerError, models::ApiResponse, AppState};
use act_domain::system_service::{MetricsSummary, PerformanceMetrics, TimePeriod, UserActivity};
use axum::{
...
pub async fn record_event(
    ...
    info!(
        ...
        request.event_type, request.event_name
    );
...
pub async fn record_session_event(
    ...
    info!(
        "Recording session event: {} for session {}",
        request.event_name, request.session_id
    );
...
pub async fn record_performance(
    ...
    let active_sessions = match state
        .domain_services
        .session_service
    ...
    let active_processes = match state
        .domain_services
        .process_service
    ...
    match state
        .domain_services
        .system_service
        .record_current_performance_metrics(active_sessions, active_processes)
        .await
    {
        Ok(()) => {
...
pub async fn get_summary(
    ...
    let start_time = params
        .start_time
        .map(|t| chrono::DateTime::from_timestamp(t, 0).unwrap_or_default())
        .unwrap_or(now - chrono::Duration::seconds(86400));
...
pub async fn get_performance_metrics(
    ...
    let (start_time_duration, period_type) = match period.as_str() {
        ...
        _ => {
            return Err(ServerError(act_core::error::CoreError::Validation(
