...
use crate::{
    models::{Session, ApiResponse, session_info_from_domain},
...
pub async fn create_session(
    ...
    let workspace_path = match state.domain_services.workspace_service
        .get_workspace(&auth_user.user_id, &request.workspace_id)
        .await
    {
        Ok(workspace) => Some(workspace.local_path),
        Err(e) => {
            warn!("Could not get workspace path for {}: {}", request.workspace_id, e);
            None
        }
    };
    ...
    let session = Session {
        ...
        shell_history: None,
        terminal_size: request.terminal_size.map(|size| serde_json::to_string(&crate::models::TerminalSize {
            cols: size.cols,
            rows: size.rows,
        }).unwrap_or_default()),
        last_command: None,
...
pub async fn get_session_buffer(
    ...
    info!("Session buffer requested: {} for user {}", session_id, auth_user.user_id);
