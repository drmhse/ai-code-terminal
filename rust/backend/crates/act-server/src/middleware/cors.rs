...
fn setup_development_cors(cors: CorsLayer, allowed_origins: Vec<String>) -> CorsLayer {
    ...
    if all_origins.iter().any(|origin| origin == "*") {
        ...
        let origins: Result<Vec<_>, _> = all_origins
            .iter()
...
fn setup_production_cors(cors: CorsLayer, allowed_origins: Vec<String>) -> CorsLayer {
    if allowed_origins.iter().any(|origin| origin == "*") {
        tracing::warn!("Production mode: Wildcard origin detected - consider using specific origins");
        cors.allow_origin(Any)
    } else {
        ...
        match origins {
            Ok(parsed_origins) => {
                tracing::info!("Production mode: Allowing specific origins: {:?}", allowed_origins);
                cors.allow_origin(parsed_origins)
            },
            Err(e) => {
