use std::sync::Arc;
use act_core::{
    Result, AuthenticatedUser, GitHubAuthService, JwtService, AuthRepository
};
...
impl AuthService {
    ...
    pub async fn get_authorization_url(&self, state: &str) -> Result<String> {
        if !self.github_service.is_configured() {
            return Err(act_core::CoreError::Configuration("GitHub OAuth is not configured".to_string()));
        }
    ...
    pub async fn handle_oauth_callback(
        ...
        let token_result = self.github_service
            .exchange_code_for_token(code, state)
            .await?;

        if !self.github_service.validate_tenant_user(&token_result.user.username) {
            return Err(act_core::CoreError::Authorization(
                format!("User {} is not authorized", token_result.user.username)
            ));
        }
    ...
    pub async fn get_auth_status(&self, user_id: &str) -> Result<AuthStatus> {
        let is_authenticated = self.auth_repository.is_github_authenticated(user_id).await?;
        
        let mut user_info = None;
        if is_authenticated {
            if let Ok(Some(access_token)) = self.auth_repository.get_github_token(user_id).await {
                if self.github_service.validate_token(&access_token).await.unwrap_or(false) {
    ...
    pub async fn validate_auth(&self, jwt_token: &str) -> Result<bool> {
        ...
        if self.auth_repository.is_github_token_expired(&claims.sub).await? {
            if let Ok(Some(refresh_token)) = self.auth_repository.get_github_refresh_token(&claims.sub).await {
                match self.github_service.refresh_access_token(&refresh_token).await {
                    Ok(new_token) => {
                        self.auth_repository.store_github_token(
                            &claims.sub,
