use act_core::{
    CopyRequest, CoreError, CreateDirectoryRequest, CreateFileRequest, DirectoryListing,
    FileContent, FileItem, FilePermissions, FileSystem, MoveRequest, Result,
};
...
impl SandboxedFileSystem {
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        ...
        if !self.workspace_root.exists() {
            async_fs::create_dir_all(&self.workspace_root)
                .await
                .map_err(|e| {
        ...
        self.workspace_root = self.workspace_root.canonicalize().map_err(|e| {
            CoreError::FileSystem(format!("Failed to canonicalize workspace root: {}", e))
        })?;
    ...
    fn resolve_path(&self, path: &Path) -> Result<PathBuf> {
        ...
        let canonical_workspace = if self.workspace_root.exists() {
            ...
            if self.workspace_root.is_absolute() {
                ...
                std::env::current_dir()
                    .map_err(|e| {
                        CoreError::FileSystem(format!("Failed to get current directory: {}", e))
                    })?
        ...
        let canonical = if resolved.exists() {
            resolved.canonicalize().map_err(|e| {
                CoreError::FileSystem(format!("Failed to canonicalize path {:?}: {}", resolved, e))
            })?
        } else {
...
impl FileSystem for SandboxedFileSystem {
    ...
    async fn read_file(&self, path: &Path) -> Result<FileContent> {
        ...
        let metadata = async_fs::metadata(&full_path)
            .await
            .map_err(|e| CoreError::FileSystem(format!("Failed to read metadata: {}", e)))?;
        ...
        if request.create_parent_dirs {
            if let Some(parent) = full_path.parent() {
                async_fs::create_dir_all(parent).await.map_err(|e| {
                    CoreError::FileSystem(format!("Failed to create parent directories: {}", e))
                })?;
            }
        }

        async_fs::write(&full_path, &request.content)
            .await
    ...
    async fn delete_directory(&self, path: &Path, recursive: bool) -> Result<()> {
        ...
        if recursive {
            async_fs::remove_dir_all(&full_path).await.map_err(|e| {
                CoreError::FileSystem(format!("Failed to delete directory recursively: {}", e))
            })?;
        } else {
    ...
    async fn move_item(&self, request: MoveRequest) -> Result<()> {
        ...
        async_fs::rename(&from_path, &to_path)
            .await
    ...
    async fn copy_item(&self, request: CopyRequest) -> Result<()> {
        ...
        debug!(
            "Copying item: {:?} -> {:?} (recursive: {})",
            from_path, to_path, request.recursive
        );
...
impl SandboxedFileSystem {
    ...
    fn copy_dir_recursive<'a>(
        ...
        Box::pin(async move {
            async_fs::create_dir_all(to)
                .await
                .map_err(|e| CoreError::FileSystem(format!("Failed to create directory: {}", e)))?;

            let mut entries = async_fs::read_dir(from)
                .await
                .map_err(|e| CoreError::FileSystem(format!("Failed to read directory: {}", e)))?;

            while let Some(entry) = entries.next_entry().await.map_err(|e| {
                CoreError::FileSystem(format!("Failed to read directory entry: {}", e))
            })? {
                let entry_path = entry.path();
                ...
                if entry_path.is_file() {
                    async_fs::copy(&entry_path, &dest_path).await.map_err(|e| {
                        CoreError::FileSystem(format!("Failed to copy file: {}", e))
                    })?;
