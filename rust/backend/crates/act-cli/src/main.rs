use act_core::repository::{CreateWorkspaceRequest, WorkspaceRepository};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let database_url =
        std::env::var("ACT_DATABASE_URL").unwrap_or_else(|_| "sqlite:./data/act.db".to_string());
    ...
    match cli.command {
        ...
        Commands::ListWorkspaces => match real_repo.list_all(&cli.user_id).await {
            Ok(workspaces) => {
