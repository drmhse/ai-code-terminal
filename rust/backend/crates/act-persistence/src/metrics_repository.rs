...
use act_domain::system_service::{
    CommandUsage, MetricEvent, MetricsRepository, MetricsSummary, PerformanceMetrics, TimePeriod,
    UserActivity,
};
...
impl MetricsRepository for SqlxMetricsRepository {
    async fn record_event(&self, event: MetricEvent) -> Result<String> {
        ...
        let properties_json = serde_json::to_string(&event.properties).map_err(|e| {
            act_core::CoreError::Database(format!("Failed to serialize properties: {}", e))
        })?;

        debug!(
            "Recording metric event: {} - {}",
        ...
        .await
        .map_err(|e| {
            act_core::CoreError::Database(format!("Failed to record metric event: {}", e))
        })?;
    ...
    async fn record_performance_metrics(&self, metrics: PerformanceMetrics) -> Result<()> {
        debug!(
            "Recording performance metrics: CPU {}%, Memory {}MB",
        ...
        .await
        .map_err(|e| {
            act_core::CoreError::Database(format!("Failed to record performance metrics: {}", e))
        })?;
    ...
    async fn get_metrics_summary(&self, time_period: TimePeriod) -> Result<MetricsSummary> {
        debug!(
            "Getting metrics summary for period: {} to {}",
        ...
        .await
        .map_err(|e| {
            act_core::CoreError::Database(format!("Failed to get average session duration: {}", e))
        })?
        .unwrap_or(0.0);
        ...
        .await
        .map_err(|e| {
            act_core::CoreError::Database(format!("Failed to get command usage: {}", e))
        })?;

        let most_used_commands = command_usage_rows
            .into_iter()
        ...
        .await
        .map_err(|e| {
            act_core::CoreError::Database(format!("Failed to get peak concurrent sessions: {}", e))
        })?
        .unwrap_or(0) as u64;
    ...
    async fn get_user_activity(&self, user_id: &str, days: i32) -> Result<UserActivity> {
        ...
        debug!(
            "Getting user activity for user {} over {} days",
            user_id, days
        );
        ...
        .await
        .map_err(|e| {
            act_core::CoreError::Database(format!("Failed to get user session count: {}", e))
        })?
        .unwrap_or(0) as u64;
        ...
        .await
        .map_err(|e| {
            act_core::CoreError::Database(format!("Failed to get user total duration: {}", e))
        })?
        .unwrap_or(0);
        ...
        .await
        .map_err(|e| {
            act_core::CoreError::Database(format!("Failed to get user command count: {}", e))
        })?
        .unwrap_or(0) as u64;
        ...
        .await
        .map_err(|e| {
            act_core::CoreError::Database(format!("Failed to get user last active: {}", e))
        })?
        ...
        .await
        .map_err(|e| {
            act_core::CoreError::Database(format!("Failed to get user favorite commands: {}", e))
        })?;

        let favorite_commands = fav_commands_rows
            .into_iter()
        ...
        .await
        .map_err(|e| {
            act_core::CoreError::Database(format!(
                "Failed to get user repositories accessed: {}",
                e
            ))
        })?
        .unwrap_or(0) as u64;
    ...
    async fn get_performance_metrics(
        ...
        debug!(
            "Getting performance metrics for period: {} to {}",
            time_period.start_time, time_period.end_time
        );
        ...
        .await
        .map_err(|e| {
            act_core::CoreError::Database(format!("Failed to get performance metrics: {}", e))
        })?;
    ...
    async fn cleanup_old_metrics(&self, days_to_keep: i32) -> Result<u64> {
        ...
        let events_deleted = sqlx::query("DELETE FROM metric_events WHERE timestamp < ?")
            ...
            .await
            .map_err(|e| {
                act_core::CoreError::Database(format!("Failed to cleanup old metric events: {}", e))
            })?
            .rows_affected();

        // Clean up old performance metrics
        let perf_deleted = sqlx::query("DELETE FROM performance_metrics WHERE timestamp < ?")
            .bind(cutoff_time)
            .execute(&*self.pool)
            .await
            .map_err(|e| {
                act_core::CoreError::Database(format!(
                    "Failed to cleanup old performance metrics: {}",
                    e
                ))
            })?
            .rows_affected();
        ...
        if total_deleted > 0 {
            info!(
                "Cleaned up {} old metric records ({} events, {} performance metrics)",
    ...
    async fn export_metrics(&self, time_period: TimePeriod, format: &str) -> Result<String> {
        debug!(
            "Exporting metrics in {} format for period: {} to {}",
        ...
        match format.to_lowercase().as_str() {
            ...
            "csv" => {
                ...
                .map_err(|e| {
                    act_core::CoreError::Database(format!(
                        "Failed to fetch metrics for CSV export: {}",
                        e
                    ))
                })?;
